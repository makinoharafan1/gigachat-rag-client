...
from transport.schemas.response import NewResponseWithData, NewResponseWithMessageERROR, NewJSONResponseOK
from transport.schemas.agent import AgentShortSchema
...
def get_agent_list_by_filters(route: str):
    ...
    def handler(repositories: Repositories, services: Services, logger: Logger, request: Request) -> Response:
        ...
        try:
            ...
            agents = [AgentShortSchema(
                id=agent.id, 
                title=agent.title,
                logo=agent.logo,
            ) for agent in agents]

            return NewResponseWithData(status_code=200, data=[agent.model_dump() for agent in agents])
...
def get_agent_by_id(route: str):
    ...
    def handler(repositories: Repositories, services: Services, logger: Logger, request: Request) -> Response:
        ...
        try:
            ...
            if agent == None :
                return NewResponseWithMessageERROR(status_code=400, message="Агент с таким id не найден")
...
def create_agent(route: str):
    ...
    async def handler(repositories: Repositories, services: Services, logger: Logger, request: Request) -> Response:
        ...
        try:
            ...
            try:
                ...
                return NewResponseWithMessageERROR(status_code=400, message="Неверный набор данных")
...
def update_agent(route: str):
    ...
    @put(route, sync_to_thread=False)
    async def handler(repositories: Repositories, services: Services, logger: Logger, request: Request) -> Response:
